...
class Statement extends CodeModel {
  ...
  Statement.assert$(Expression expression, {String? message})
    : this([
        ...
        SeparatedValues.forParameters([
          expression,
          if (message != null) Expression.ofString(message),
        ]),
        Code(')'),
      ]);
  ...
  Statement.ifChain$(
    Map<Expression, Block> conditionsAndBlocks, {
    Block? elseBlock,
  ...
  Statement.library(String name)
    : this([KeyWord.library$, Space(), IdentifierStartingWithLowerCase(name)]);
  ...
  Statement.switch$(
    Expression condition,
    Map<Expression, Block> valuesAndBlocks, {
    Block? defaultBlock,
  }) : this(_createSwitchNodes(condition, valuesAndBlocks, defaultBlock));

  static List<CodeNode> _createSwitchNodes(
    Expression condition,
    Map<Expression, Block> valuesAndBlocks,
    Block? defaultBlock,
  ) {
  ...
  Statement.throw$(Expression expression)
    : this([KeyWord.throw$, Space(), expression]);

  Statement.try$(
    Block tryBlock, {
    List<Catch> catches = const [],
    Block? finallyBlock,
  }) : this([
         KeyWord.try$,
         Space(),
